...
resource "aws_eks_cluster" "aws_eks" {
  ...
  vpc_config {
    subnet_ids = ["subnet-0c0a6eb41d9d7b163","subnet-081964b505bcc0f88","subnet-0f1b6e923eb3a813a"]
  }
...
resource "aws_eks_node_group" "node1" {
  ...
  remote_access {
   ec2_ssh_key = "nasik"
   source_security_group_ids = ["sg-03fed34d2e55f87de"]
  }
  scaling_config {
    desired_size = 1
    max_size     = 1
...
resource "aws_efs_mount_target" "subnet1" {
  ...
  subnet_id = "subnet-0c0a6eb41d9d7b163"
  security_groups = ["sg-03fed34d2e55f87de"]
}

resource "aws_efs_mount_target" "subnet2" {
  file_system_id = aws_efs_file_system.efs.id
  subnet_id = "subnet-081964b505bcc0f88"
  security_groups = ["sg-03fed34d2e55f87de"]
}

resource "aws_efs_mount_target" "subnet3" {
  file_system_id = aws_efs_file_system.efs.id
  subnet_id = "subnet-0f1b6e923eb3a813a"
  security_groups = ["sg-03fed34d2e55f87de"]
}
